...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"parallax-mouse.js":[function(require,module,exports) {
"use strict";
...
function parallaxMouse(options) {
  ...
  document.querySelector(wrap).addEventListener('mousemove', function (e) {
    var percentX = (0 - e.pageX / window.innerWidth * moveFactor - moveFactor / 2 + moveFactor) / 2;
    var percentY = (0 - e.pageY / window.innerHeight * moveFactor - moveFactor / 2 + moveFactor) / 2;
    elements.forEach(function (element) {
      element.style.transform = "translate(".concat(percentX, "%, ").concat(percentY, "%)");

      if (perspective) {
        var mouseX = (e.pageX - window.pageYOffset - window.innerWidth / 2) / window.innerWidth;
        var mouseY = (e.pageY - window.pageXOffset - window.innerHeight / 2) / window.innerWidth;
        element.style.transform += "rotateX(".concat(mouseX * 30, "deg) rotateY(").concat(mouseY * -30, "deg) perspective(").concat(perspective, ")");
      }
    });
  });
}
